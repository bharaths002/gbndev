...
import aboutagencyImg from "/aboutagency.jpg";
...
const App: React.FC = () => {
  ...
  // Smooth scroll handler — replaces raw window.location.href = "#section"
  ...
  return (
    <div className="bg-bone min-h-screen text-obsidian">
      {/* ✅ Navbar manages its own mobile state internally — no props needed */}
      <Navbar />
      ...
      >
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
            {/* LEFT — Content
                pt-16 lg:pt-0 — extra top padding on mobile so "Clarity" heading
                doesn't overlap the fixed navbar. Removed on lg+ screens.
                👉 Increase to pt-20 or pt-24 if still overlapping on your device */}
            <div className="flex flex-col justify-center items-center text-center space-y-8 text-left pt-16 lg:pt-0">
              <h1 className="text-6xl md:text-7xl xl:text-8xl font-serif leading-tight animate-slide-up">
                Clarity <br />
            ...
            <div className="relative flex items-center justify-center animate-slide-up [animation-delay:300ms]">
              <div className="group relative w-full max-w-[75%] aspect-[3/4] max-h-[60vh] rounded-3xl overflow-hidden mx-auto">
                <img
                  src={homepageImg}
                  alt="homepageimg"
            ...
            <div className="reveal">
              ...
              <div className="space-y-6 text-base text-obsidian/70 leading-relaxed">
                ...
                <p>
                  ...
                  combine deep editorial expertise with modern data to ensure
                  your work is not only perfect on the page but also impactful
                  in the marketplace.
                </p>
                <p>
                  Whether you are a novelist finishing your first draft or a
                  business scaling its content footprint, we weave stories
                  together to help you stand out.
                </p>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {LITERARY_PROCESS.map((step, i) => (
              ...
              >
                <div className="p-8 h-full bg-white rounded-2xl border border-black/5 shadow-sm hover:shadow-md transition-all duration-300">
                  <div className="w-12 h-12 rounded-full bg-obsidian/5 flex items-center justify-center mb-6 text-crystal group-hover:bg-crystal group-hover:text-white transition-all duration-300">
                    <span className="inline-flex group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300">
                      {step.icon}
                    </span>
                  </div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {FEATURES.map((feature, i) => (
              <div
                key={i}
                className="reveal group p-10 rounded-2xl border border-black/5 bg-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col"
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                <div className="w-12 h-12 rounded-lg bg-obsidian text-white flex items-center justify-center mb-8 group-hover:bg-crystal transition-colors duration-300">
                  <span className="inline-flex group-hover:scale-125 group-hover:-rotate-12 transition-transform duration-300">
                    {feature.icon}
                  </span>
                </div>
                <h3 className="text-xl font-bold mb-4">{feature.title}</h3>
                <p className="text-obsidian/60 leading-relaxed text-sm mb-6">
                  {feature.description}
                </p>
      ...
      {/* Contact Section */}
      {/* ─── PASTE THIS ENTIRE BLOCK to replace your existing Contact Section ─── */}
      <section id="contact" className="py-24 bg-white">
        <div className="container mx-auto px-4 sm:px-6 max-w-5xl">
          ...
          <div className="bg-silver/30 rounded-3xl p-6 sm:p-10 md:p-16 border border-black/5 overflow-visible relative">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 relative z-10">
              ...
              <div className="reveal">
                ...
                <p className="text-obsidian/50 mb-10 leading-relaxed text-sm md:text-base">
                  ...
                  Reach out today for a consultation. We respond to all inquiries
                  within 24 hours.
                </p>
                <div className="space-y-4 text-sm">
                  <div className="flex items-center gap-4 text-obsidian/70">
                    <span className="w-8 h-8 shrink-0 rounded-full bg-white border border-black/5 flex items-center justify-center">
                      @
                    </span>
                    <span className="break-all">glassbonescreative@gmail.com</span>
                  </div>
                  <div className="flex items-center gap-4 text-obsidian/70">
                    <span className="w-8 h-8 shrink-0 rounded-full bg-white border border-black/5 flex items-center justify-center">
                      I
              ...
              {/* Right — Form with custom dropdown */}
              {(() => {
                // ─── Custom dropdown state lives inside an IIFE so it doesn't
                //     pollute the outer component scope. If you prefer, move
                //     selectedService / serviceOpen / setters to the top of App.tsx.
                ...
                // ✅ CHANGE 1: Full services list — all 7 services added
                ...
                return (
                  <form className="reveal [transition-delay:200ms] space-y-4">
                    ...
                    {/* ✅ CHANGE 2: Custom animated dropdown replaces native <select>
                    Native <select> can't be animated or styled cross-browser.
                    This custom dropdown uses max-height transition for smooth open/close.
                    👉 To change animation speed: edit `duration-300` on the inner list div
                    👉 To change dropdown max height (how many items show before scroll):
                       change `max-h-48` to `max-h-64` or `max-h-56`
                    👉 To adjust chevron position: change `right-4` on the svg wrapper */}
                    <div className="relative" ref={dropdownRef}>
                      ...
                      >
                        ...
                        {/* ✅ CHANGE 3: Chevron icon — positioned with `mr-1` to sit
                        slightly left of the edge rather than flush right.
                        Rotates 180° when open.
                        👉 To move it more left: increase `mr-1` to `mr-2` or `mr-3` */}
                        <svg
                          className={`w-4 h-4 text-obsidian/50 mr-1 shrink-0 transition-transform duration-300 ${serviceOpen ? "rotate-180" : "rotate-0"}`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={2}
                      ...
                      {/* Dropdown list — animates with max-height + opacity
                      👉 To change background: edit `bg-white`
                      👉 To change item hover color: edit `hover:bg-crystal/10` */}
                      <div
                        className={`absolute left-0 right-0 top-full mt-1 bg-white border border-black/10 rounded-xl shadow-lg z-50 overflow-hidden transition-all duration-300 ease-in-out ${serviceOpen ? "max-h-56 opacity-100" : "max-h-0 opacity-0"
                          }`}
                      >
                        <div className="overflow-y-auto max-h-56 py-1">
                          {services.map((service) => (
                            <button
                              ...
                              className={`w-full text-left px-5 py-3 text-sm transition-colors duration-150 hover:bg-crystal/10 hover:text-crystal ${selectedService === service
                                  ? "text-crystal font-semibold bg-crystal/5"
                                  : "text-obsidian/80"
                                }`}
                            >
                              {service}
                    ...
                    {/* Hidden input so the selected value is submitted with the form */}
            ...
            {/* Social Icons
            ✅ CHANGE 4: Changed from `flex` to `flex flex-wrap` so icons
            wrap to a second line on small screens instead of overflowing.
            Added `justify-center sm:justify-start` so they centre on mobile
            and left-align on larger screens.
            👉 To always center: remove `sm:justify-start`
            👉 To always left-align: remove `justify-center` */}
