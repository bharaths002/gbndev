  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled
          ? "bg-white/80 backdrop-blur-xl py-4 border-b border-black/5"
          : "bg-transparent py-8"
      }`}
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        >
          <div className="relative w-10 h-10 border border-obsidian/10 rounded-lg flex items-center justify-center overflow-hidden">
            ...
            <span className="relative text-xs font-bold text-obsidian group-hover:text-white transition-colors">
              GCN
            </span>
          </div>
          <span className="text-3xl  md:text-4xl font-serif tracking-tight text-obsidian">Glassbones Creative Nexus</span>
        ...
        <div className="hidden md:flex items-center space-x-8">
          {NAV_LINKS.map((link) => (
            <a
              ...
              className="text-xs md:text-sm uppercase tracking-widest text-obsidian/60 hover:text-obsidian transition-all duration-300 relative group"
            >
              {link.label}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed inset-0 top-[70px] bg-white transition-all duration-500 transform ${
        ...
        <div className="flex flex-col items-center justify-center h-full space-y-10">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-3xl font-serif text-obsidian/70 hover:text-crystal"
              onClick={(e) => handleLinkClick(e, link.href)}
            >
              {link.label}
            </a>
